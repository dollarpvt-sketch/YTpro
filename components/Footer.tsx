...
const Footer: React.FC<FooterProps> = ({ onLinkClick }) => {
    ...
    const footerLinks = {
        'Công cụ': [
            { name: 'Tạo Ảnh AI', href: '#image-generator' },
            { name: 'Tạo Video AI', href: '#video-generator' },
            { name: 'Viết Kịch Bản', href: '#script-writer'},
            { name: 'Viết Lại Kịch Bản', href: '#script-rewriter'},
            { name: 'Tut & Trick', href: '#youtube-tricks'},
            { name: 'Tìm Kênh Ẩn', href: '#channel-discovery'},
            { name: 'Chuyển đổi Giọng Nói', href: '#text-to-speech'},
            { name: 'AI Automation', href: '#automation'},
        ],
        'Sản phẩm': [
            { name: 'Tất cả công cụ', href: '#tools' },
            { name: 'Đánh giá', href: '#testimonials' },
            { name: 'Giá', href: '#pricing' },
            { name: 'Affiliate', href: '#affiliate' },
        ],
        'Công ty': [
            { name: 'Về chúng tôi', href: '#', pageId: 'about' },
            { name: 'Liên hệ', href: '#', pageId: 'contact' },
            { name: 'Blog', href: '#', pageId: 'blog' },
        ],
        'Pháp lý': [
            { name: 'Điều khoản dịch vụ', href: '#', pageId: 'terms' },
            { name: 'Chính sách bảo mật', href: '#', pageId: 'privacy' },
        ],
    };
                                ...
                                {links.map(link => (
                                    <li key={link.name}>
                                        <a 
                                          ...
                                          className="text-text-secondary hover:text-text-main transition-colors duration-300 cursor-pointer"
                                        >
                                          {link.name}
                                        </a>
                                    </li>
