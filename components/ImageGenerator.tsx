                                        ...
                                        generatedImages.map(image => (
                                            <div key={image.id} className="relative group aspect-square">
                                                ...
                                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl flex items-center justify-center gap-2">
                                                    <a href={image.url} download={`yt-pro-vision-${Date.now()}.jpg`} title="Tải xuống" className="p-3 bg-white/10 rounded-full text-white hover:bg-white/20"><DownloadIcon className="w-5 h-5"/></a>
                                                </div>
                                            </div>
                                        ))
                                    ) : (
                ...
                {history.length > 0 && (
                    <div className="mt-8">
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                            {history.map(img => (
                                <img key={img.id} src={img.url} alt={img.prompt} title={img.prompt} className="w-full aspect-square object-cover rounded-lg cursor-pointer hover:scale-110 transition-transform duration-200" />
                            ))}
                        </div>
                    </div>
                )}
